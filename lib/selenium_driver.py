...
class Driver:
    ...
    def find_elements(
        self, *locator: tuple, timeout: float = timeout_sec, wait: bool = True
    ) -> List[WebElement]:
        if wait:
            self.wait_until_element_is_present(locator, timeout)
    ...
    def click_element_with_js(self, *locator: tuple):
        element = self.find_element(*locator)
        self.driver.execute_script("arguments[0].click()", element)

    # Click on web element and check that another element appear
    def click_and_check(
        self, click_locator: tuple, check_locator: tuple, timeout: float = timeout_sec
    ):
        ...
        raise NoSuchElementException(
            f'Element - "{check_locator}" did not appear after clicking "{click_locator}"'
        )

    # Click on web element and check that element missed
    def click_and_wait_for_element_to_be_hidden(
        self, *locator: tuple, timeout: int = timeout_sec
    ):
    ...
    def click_and_wait_for_element_to_be_not_clickable(
        self, *locator: tuple, timeout: int = timeout_sec
    ):
        self.click(*locator, timeout=timeout)
        for _ in range(3):
    ...
    def click_and_wait_for_other_element_to_be_hidden(
        self, click_locator: tuple, check_locator: tuple, timeout: int = timeout_sec
    ):
        self.click(*click_locator, timeout=timeout)
        for _ in range(3):
            try:
                ...
                self.click(*click_locator)
        raise Exception(
            f'Element - "{check_locator}" did not missed after clicking - "{click_locator}"'
        )
    ...
    def click_child_element(self, parent_element: WebElement, child_locator: tuple):
        try:
            ...
            if type(
                exception
            ) == WebDriverException and "is not clickable at point" not in str(
                exception
            ):
                raise exception
            else:
    ...
    @allure.step(
        'Type text "{text}" into element - {locator} without waiting and clearing'
    )
    ...
    def is_element_invisible(
        self, *locator: tuple, timeout: float = timeout_sec
    ) -> bool:
        try:
    ...
    def get_text(
        self, *locator: tuple, wait: bool = True, timeout: float = timeout_sec
    ) -> str:
        if wait:
            self.wait_until_element_is_visible(locator, timeout=timeout)
    ...
    def get_text_of_many_elements(
        self, *locator: tuple, wait: bool = True, timeout: float = timeout_sec
    ) -> list:
        return [
            element.text
            for element in self.find_elements(*locator, timeout=timeout, wait=wait)
        ]

    # Get attribute needed value from
    @allure.step('Get attribute "{attribute}" value of element - {locator}')
    def get_attribute_value(
        self, attribute: str, *locator: tuple, timeout: float = timeout_sec
    ) -> str:
    ...
    def get_all_options_from_lazy_loading_select(
        self, options_locator: tuple, wait: bool = True
    ) -> List[str]:
        # For animation
        time.sleep(1)
        options = []
    ...
    def wait_until_element_is_present(
        ...
        try:
            ...
            raise NoSuchElementException(
                f'Element "{locator}" not present in DOM after {timeout} seconds'
            )
        except NoSuchWindowException:
            self.avoid_no_such_window(
                locator, ec.presence_of_element_located, until=True, timeout=timeout
            )
    ...
    def wait_until_element_is_hidden(
        ...
        try:
            ...
            raise NoSuchElementException(
                f'Element "{locator}" was present for too long. '
                f"It takes more than {timeout} sec to hide an element"
            )
        except NoSuchWindowException:
            self.avoid_no_such_window(
                locator, ec.presence_of_element_located, until=False, timeout=timeout
            )

    def wait_until_element_is_not_clickable(
        self, locator: tuple, timeout: float = timeout_sec
    ):
        try:
            WebDriverWait(self.driver, timeout).until_not(
    ...
    def wait_until_element_is_visible(
        ...
        try:
            ...
            self.avoid_no_such_window(
                locator, ec.visibility_of_element_located, until=True, timeout=timeout
            )
    ...
    def wait_until_element_is_clickable(
        ...
        try:
            ...
            self.avoid_no_such_window(
                locator, ec.element_to_be_clickable, until=True, timeout=timeout
            )
    ...
    def wait_until_invisibility_of_element_located(
        self, locator: tuple, timeout: float = timeout_sec
    ):
        try:
            WebDriverWait(self.driver, timeout).until(
            ...
            self.avoid_no_such_window(
                locator, ec.invisibility_of_element_located, until=True, timeout=timeout
            )

    def wait_until_element_is_disabled(
        self, *locator: tuple, timeout: int = timeout_sec
    ):
        for _ in range(timeout):
            if self.is_element_enabled(*locator, timeout=1):
                ...
                return
        raise Exception(
            f"Element {locator} has not be disabled during {timeout} seconds"
        )
...
""",
    ...
    def avoid_no_such_window(
        self, locator: tuple, statement, until: bool, timeout: float = timeout_sec
    ):
        for _ in range(5):
            try:
                if until:
